...
void setup() {
 ...
 for(pinActual=0; pinActual<(sizeof(entradasDip));pinActual++)
   {
       pinMode (entradasDip[pinActual],INPUT_PULLUP);//Entradas para el dipSwitch
   }
    pinMode (TINI,INPUT_PULLUP);  //Entradas para comenzar el programa
    pinMode (TISENT,INPUT_PULLUP);//Entrada para definir el sentido
...
//Convertimos el arreglo int binario a un número decimal
  ...
  for(int i=7; i>=0; i--)
  {
    conversion=(res*2)+dipSInt[i];//Número decimal que obtenemos
    res=conversion;
  }
